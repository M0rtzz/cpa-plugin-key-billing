...
func TestRequestEventLobotomizedFilter(t *testing.T) {
	database, state := requestEventDatabase(t)
	turnState := func(entry billing.RequestEvent, length int) billing.RequestEvent {
		entry.ResponseHeaders = http.Header{billing.CodexTurnStateHeader: {strings.Repeat("s", length)}}
		return entry
	}
	degraded := turnState(requestEvent("scope-a", eventStart.Add(6*time.Minute)), billing.LobotomizedTurnStateLength)
	healthy := turnState(requestEvent("scope-a", eventStart.Add(7*time.Minute)), 128)
	degradedFailure := turnState(requestEvent("scope-b", eventStart.Add(8*time.Minute)), billing.LobotomizedTurnStateLength)
	mustSave(t, database, state, billing.Changes{
		NormalRequestEvents: []billing.RequestEvent{degraded, healthy},
		RequestErrorEvents:  []billing.RequestErrorEvent{{Event: degradedFailure}},
	})

	view := mustQueryRequestEvents(t, database, billing.RequestEventQuery{})
	// Degraded turns overlap the normal and failed counts instead of splitting them.
	if view.Statuses != (billing.RequestEventStatusCounts{All: 9, Normal: 6, Failed: 3, Lobotomized: 2}) {
		t.Fatalf("statuses = %+v", view.Statuses)
	}
	if row := view.Entries[1]; row.Lobotomized {
		t.Fatalf("healthy turn state = %+v", row)
	}
	// Events recorded before the header snapshot existed report no turn state.
	if row := view.Entries[len(view.Entries)-1]; row.Lobotomized {
		t.Fatalf("event without response headers = %+v", row)
	}

	filtered := mustQueryRequestEvents(t, database, billing.RequestEventQuery{Lobotomized: true})
	if filtered.Total != 2 || len(filtered.Entries) != 2 || filtered.Statuses != view.Statuses {
		t.Fatalf("filtered = %+v of %d, statuses %+v", filtered.Entries, filtered.Total, filtered.Statuses)
	}
	for i, want := range []billing.RequestEvent{degradedFailure, degraded} {
		if row := filtered.Entries[i]; !row.Lobotomized || !row.At.Equal(want.At) || row.Failed != (i == 0) {
			t.Fatalf("filtered entry %d = %+v", i, row)
		}
	}
	normal, failed := false, true
	for _, scenario := range []struct {
		query  billing.RequestEventQuery
		total  int
		counts billing.RequestEventStatusCounts
	}{
		{billing.RequestEventQuery{Failed: &normal}, 6, view.Statuses},
		{billing.RequestEventQuery{Failed: &failed}, 3, view.Statuses},
		{billing.RequestEventQuery{Scope: "scope-a", Lobotomized: true}, 1,
			billing.RequestEventStatusCounts{All: 6, Normal: 6, Lobotomized: 1}},
		{billing.RequestEventQuery{KeyScope: "scope-b", Model: "gpt-5.5", Provider: "codex",
			Executor: "CodexExecutor", From: degradedFailure.At, Lobotomized: true}, 1,
			billing.RequestEventStatusCounts{All: 1, Failed: 1, Lobotomized: 1}},
	} {
		got := mustQueryRequestEvents(t, database, scenario.query)
		if got.Total != scenario.total || len(got.Entries) != scenario.total || got.Statuses != scenario.counts {
			t.Fatalf("query %+v: %+v", scenario.query, got)
		}
	}
	mustSave(t, database, state, billing.Changes{NormalRequestEvents: []billing.RequestEvent{degraded}})
	page := mustQueryRequestEvents(t, database, billing.RequestEventQuery{
		SnapshotID: &filtered.SnapshotID, Lobotomized: true, Offset: 1, Limit: 1,
	})
	if page.Total != 2 || page.Statuses != filtered.Statuses || !reflect.DeepEqual(page.Entries, filtered.Entries[1:]) {
		t.Fatalf("late insertion changed filtered snapshot: %+v", page)
	}
	if fresh := mustQueryRequestEvents(t, database, billing.RequestEventQuery{Lobotomized: true}); fresh.Total != 3 {
		t.Fatalf("fresh snapshot missed insertion: %+v", fresh)
	}

}

func TestRequestEventsPersistOnlyListedResponseHeaders(t *testing.T) {
	database, state := requestEventDatabase(t)
	for _, scenario := range []struct {
		name    string
		headers http.Header
		...
		{"filtered", http.Header{
			"x-codex-turn-state":            {"", "  ", "  turn-state  ", "ignored"},
			"Set-Cookie":                    {"session=dummy-secret"},
			"Authorization":                 {"Bearer dummy-token"},
			"X-Api-Key":                     {"dummy-api-key"},
			"X-Codex-Turn-Metadata-Unknown": {"unrelated"},
		}, `{"X-Codex-Turn-State":"turn-state"}`},
		{"unreported", nil, "{}"},
		{"empty", http.Header{billing.CodexTurnStateHeader: {"", "  "}}, "{}"},
		{"duplicate-case", http.Header{
			billing.CodexTurnStateHeader: {"first"}, "x-codex-turn-state": {"second"},
		}, `{"X-Codex-Turn-State":"first"}`},
	} {
		t.Run(scenario.name, func(t *testing.T) {
			entry := requestEvent("scope-a", eventStart.Add(6*time.Minute))
			entry.ResponseHeaders = scenario.headers
			mustSave(t, database, state, billing.Changes{NormalRequestEvents: []billing.RequestEvent{entry}})
			var stored string
			if err := database.db.QueryRow("SELECT response_headers_json FROM request_events ORDER BY id DESC LIMIT 1").Scan(&stored); err != nil {
				t.Fatal(err)
			}
			if stored != scenario.want {
				t.Fatalf("stored headers = %s, want %s", stored, scenario.want)
			}
		})
	}
}

func TestRequestEventLobotomizedJSONSemantics(t *testing.T) {
	database, _ := requestEventDatabase(t)
	for _, scenario := range []struct {
		name, raw string
		want      bool
		...
		{"null-value", `{"X-Codex-Turn-State":null}`, false},
		{"empty", `{"X-Codex-Turn-State":""}`, false},
		{"unicode-312-characters", `{"X-Codex-Turn-State":"` + strings.Repeat("界", 312) + `"}`, true},
		{"unicode-312-bytes", `{"X-Codex-Turn-State":"` + strings.Repeat("界", 104) + `"}`, false},
	} {
		t.Run(scenario.name, func(t *testing.T) {
			if _, err := database.db.Exec("UPDATE request_events SET response_headers_json=? WHERE id=1", scenario.raw); err != nil {
				t.Fatal(err)
			}
			view := mustQueryRequestEvents(t, database, billing.RequestEventQuery{})
			row := view.Entries[len(view.Entries)-1]
			if row.Lobotomized != scenario.want {
				t.Fatalf("lobotomized = %v", row.Lobotomized)
			}
			filtered := mustQueryRequestEvents(t, database, billing.RequestEventQuery{Lobotomized: true})
			wantCount := 0
			if scenario.want {
			...
			if filtered.Total != wantCount || len(filtered.Entries) != wantCount || view.Statuses.Lobotomized != wantCount {
				t.Fatalf("inconsistent count/filter: %+v / %+v", view.Statuses, filtered)
			}
			...
			if string(payload["lobotomized"]) != map[bool]string{true: "true", false: "false"}[scenario.want] ||
				strings.Contains(string(raw), "response_headers") || strings.Contains(string(raw), "Turn-State") {
				t.Fatalf("unexpected response payload: %s", raw)
			}
		})
	}
	if _, err := database.db.Exec("UPDATE request_events SET response_headers_json=NULL WHERE id=1"); err == nil {
		t.Fatal("response_headers_json must reject SQL NULL")
	}
}
