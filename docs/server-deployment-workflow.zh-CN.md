...
本次插件使用 SQLite schema 20，新增可空流式状态和 Token 明细快照；启动时会从 v19 及支持的旧版本事务迁移，历史缺失字段保持未知，不改写历史金额。迁移前先在隔离副本验证，再于 CPA 完全停止后备份数据库，包括存在时的 `-wal` 和 `-shm` 文件。仅支持 v19 或更早结构的旧计费插件不能直接打开新 schema。
