...
本功能包含两个独立页面：`usage.html` 查询本人 Key 的用量和费用，`quota.html` 展示本人 Key 额度、上游共享额度和匿名账号。Plus、Pro 20x 等账号类型复用计费插件已有的配额结果。

页面不需要管理密码。用户输入自己的 CPA API Key，凭据仅保存在当前页面内存，切换页面、刷新或清除后需要重新输入。正式用户入口应通过 HTTPS 提供。
...
- 未查询：显示“待管理员更新”。
- 超过 60 秒：标记为历史快照并显示更新时间，不自动查询上游。
