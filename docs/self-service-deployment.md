...
插件代码使用数据库结构 v20，并会从 v19 及支持的旧版本事务迁移。v20 新增可空流式状态及标准化 Token 明细快照；历史缺失字段保持未知，失败和零用量记录均保留，不补猜数据或改价。结构升级保留费用金额；历史金额换算是独立的离线操作，见 [倍率与历史账单换算](billing-multiplier.md)。

仅支持 v19 或更早结构的旧插件无法打开 v20 数据库。上线前应在隔离副本上完成迁移与验收；尚未接收新请求时，可停服后恢复完整的插件、配置和数据库备份。已经产生新账单后，保留当前数据库修复，不能直接替换旧插件，也不能用旧数据库备份覆盖新增记录。不要手动修改 `PRAGMA user_version` 伪装数据库版本。
