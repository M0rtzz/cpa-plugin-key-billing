...
用量页提供模型搜索、执行类型筛选和手动刷新。类型来自宿主用量记录：明确的 WebSocket 执行器显示 WS，已上报的流式状态显示流式或同步，缺失时显示未知。模型和类型筛选仅影响明细表，顶部统计仍按日期范围展示；原生压缩、Live、安全策略分类暂不展示。

Token 与费用旁的信息按钮支持悬浮、键盘聚焦和手机点击。金色徽标动态展示账单的服务档位倍率（如 2、2.5、0.5，1 隐藏），长上下文使用独立标记。费用与实际单价已含倍率；全局倍率前金额由入账金额除以该账单的全局倍率换算。CPA v7.3.17 未提供图片 Token 和费用细分，生图模型仅显示标识及“细分未上报”，不把全部输出当作图片 Token。
...
插件代码使用数据库结构 v21，并会从 v20 及支持的旧版本事务迁移。v21 新增可空的宿主请求模型及上游执行模型，用于管理员模型分布；历史缺失保持未知，不反推模型映射。v20 新增可空流式状态及标准化 Token 明细快照；历史缺失字段保持未知，失败和零用量记录均保留，不补猜数据或改价。结构升级保留费用金额；历史金额换算是独立的离线操作，见 [倍率与历史账单换算](billing-multiplier.md)。

仅支持 v20 或更早结构的旧插件无法打开 v21 数据库。上线前应在隔离副本上完成迁移与验收；尚未接收新请求时，可停服后恢复完整的插件、配置和数据库备份。已经产生新账单后，保留当前数据库修复，不能直接替换旧插件，也不能用旧数据库备份覆盖新增记录。不要手动修改 `PRAGMA user_version` 伪装数据库版本。


## 管理员分析页

“全部 API Key”支持自动、按小时、按天的时间粒度（最多 1,000 桶），模型的计费/请求/上游/映射分布、Token 与缓存命中率趋势，以及按区间总 Token 选出的 Top 10 Key 趋势。模型分布默认使用账单已保存的计费模型，兼容历史记录；各维度显示字段覆盖数量。原始请求/上游字段缺失时保持未知，不用计费模型回填映射，也不修改历史金额或数据库结构。端点路径未由 CPA v7.3.17 用量接口提供，端点模块仅说明能力限制。

用量明细、错误请求和用户排行均支持分页；单 Key 隐藏全局图表及用户排行。右上角 Key 和时间影响整个分析页，模型/执行类型/状态仅筛选明细表。倍率前金额逐笔按历史全局倍率换算，不代表真实上游成本。

“导出 Excel”输出当前标签全部筛选结果，固定快照，支持取消；文件仅使用脱敏 Key，不包含完整密钥。SheetJS CE 0.20.3 随插件嵌入，无运行时 CDN 依赖，其许可证保存在 `internal/plugin/vendor/SheetJS-LICENSE.txt`。
