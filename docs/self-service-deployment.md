...
共享库必须与服务器的操作系统、架构及 libc 兼容。异机部署时，在兼容的 Linux 环境构建；可复用仓库发布工作流的 manylinux 构建环境。CPA 使用支持插件的版本，已支持的最低版本为 7.2.143，当前服务器为 v7.3.17，已提供插件响应档位和模型字段。本次插件为 GPT-5.6/GPT-6 系列的 Codex OAuth Priority 应用 2× 倍率，其他模型保持 2.5×；仅升级 CPA 不会自动改变插件规则。
...
本次增加服务档位价格的插件代码使用数据库结构 v19，并会从 v18 及支持的旧版本迁移。结构升级保留费用金额；历史金额换算是独立的离线操作，见 [倍率与历史账单换算](billing-multiplier.md)。

仅支持 v18 的旧插件无法打开 v19 数据库。上线前应在隔离副本上完成迁移与验收；尚未接收新请求时，可停服后恢复完整的插件、配置和数据库备份。已经产生新账单后，保留当前数据库修复，不能直接替换旧插件，也不能用旧数据库备份覆盖新增记录。不要手动修改 `PRAGMA user_version` 伪装数据库版本。
